# 基金筛选系统

自用基金筛选工具，重点筛选稳健增长、长期复利的基金，支持定投策略。

> **当前版本**：v1.7.0  
> **最后更新**：2026-02-23

## 版本记录

| 版本 | 日期 | 修改内容 |
|------|------|---------|
| v1.0.0 | 2026-02-17 | 初始版本，完成MVP核心功能 |
| v1.1.0 | 2026-02-17 | 新增企业微信机器人推送支持 |
| v1.2.0 | 2026-02-17 | 新增邮件推送功能，支持SMTP协议 |
| v1.3.0 | 2026-02-23 | 新增基金经理维度分析，综合评分引入经理评分 |
| v1.4.0 | 2026-02-23 | 新增量化基金识别功能，优化数据库批量写入性能 |
| v1.5.0 | 2026-02-23 | 新增多线程并行抓取，update-funds 支持更新已存在基金 |
| v1.6.0 | 2026-02-23 | 新增 update-nav 命令，批量并行更新基金净值数据 |
| v1.7.0 | 2026-02-24 | run-all自动更新数据，筛选配置.env化（基金类型/收益年限/收益率等） |

## 功能特性

- 📊 **全市场基金筛选**：从所有公募基金中筛选优质标的
- 📈 **多维度指标**：夏普比率、最大回撤、卡玛比率、月度胜率
- 🎯 **综合评分**：多因子加权打分，量化基金质量
- 👤 **基金经理分析**：引入基金经理历史表现评分，关注"人"的因素
- 🤖 **量化基金识别**：自动识别量化策略基金，区分投资风格
- 📉 **历史回测**：对比沪深300等基准，查看超额收益
- 📱 **多渠道推送**：支持 Server酱、企业微信机器人、邮件推送
- ⏰ **定时任务**：支持定时自动筛选和推送

## 快速开始

### 1. 安装依赖

```bash
# 使用 uv 安装依赖（推荐）
uv sync

# 或使用传统方式
uv venv
uv pip install -e .
```

### 2. 配置环境变量

```bash
# 复制示例配置文件
cp .env.example .env

# 编辑 .env 文件，配置推送渠道（至少配置一个）
vim .env
```

**推送渠道配置（至少配置一个）：**

| 渠道 | 获取方式 | 环境变量 |
|------|---------|---------|
| Server酱 | 访问 https://sct.ftqq.com/ 注册获取SCKEY | `SERVER_CHAN_KEY` |
| 企业微信 | 在企业微信群中添加机器人，复制Webhook地址 | `WECOM_WEBHOOK` |
| 邮件 | 配置SMTP服务（Gmail/QQ邮箱/163邮箱等） | `SMTP_HOST`, `SMTP_USER`, `SMTP_PASSWORD`, `EMAIL_RECEIVER` |

**邮件配置提示**：
- Gmail：需要在账户设置中生成应用专用密码
- QQ邮箱：需要在设置中开启SMTP服务并获取授权码
- 163/126邮箱：需要在设置中开启SMTP服务并获取授权码
- **多收件人**：`EMAIL_RECEIVER` 支持多个邮箱，用英文逗号分隔
  ```
  EMAIL_RECEIVER=user1@example.com,user2@example.com
  ```

### 3. 初始化数据库

```bash
# 初始化数据库表
python main.py init
```

### 4. 首次运行

```bash
# 方案一：使用脚本命令（推荐）
uv run fund-screener run-all

# 方案二：使用模块方式
uv run python -m fund_screener run-all

# 方案三：分步执行
uv run fund-screener update-funds  # 更新基金列表
uv run fund-screener screen        # 执行筛选
uv run fund-screener backtest      # 执行回测
uv run fund-screener notify        # 推送报告
```

## 使用指南

### 命令说明

```bash
# 初始化数据库
uv run fund-screener init

# 更新基金基础数据（首次运行需要）
uv run fund-screener update-funds

# 更新基金净值数据（批量并行抓取，默认10线程）
uv run fund-screener update-nav
uv run fund-screener update-nav --workers 20  # 自定义线程数
uv run fund-screener update-nav --limit 100 --workers 10  # 限制100只基金
uv run fund-screener update-nav --force  # 强制全量更新（忽略已有数据）

# 执行筛选
uv run fund-screener screen

# 执行回测
uv run fund-screener backtest

# 推送报告到微信/企业微信
uv run fund-screener notify

# 测试推送功能（验证Server酱、企业微信、邮件配置）
uv run fund-screener test-notify

# 完整流程（更新数据+筛选+回测+推送）
uv run fund-screener run-all

# 启动定时任务（每周一 09:00 自动执行）
uv run fund-screener schedule
```

### 筛选标准

系统会从以下几个维度评估基金：

**硬性门槛**：
- 成立时间 > 3年
- 基金规模 > 2亿
- 最大回撤 < 25%
- 夏普比率 > 0.8
- 近3年年化收益 > 5%

**评分权重**：
- 3年年化收益：25%
- 夏普比率：20%
- 卡玛比率：15%
- 月度胜率：10%
- 回撤控制：10%
- **基金经理评分：20%**

> 💡 基金经理评分基于该经理历史管理的基金表现计算，包括：
> - 管理基金数量（越多经验越丰富）
> - 管理基金的平均夏普比率
> - 管理基金的最大回撤控制能力

> ⚙️ **筛选参数可在 `.env` 中配置**：
> - `SCREEN_FUND_TYPES`: 基金类型（全部/混合型/股票型/债券型等）
> - `SCREEN_RETURN_YEARS`: 收益计算年限
> - `SCREEN_MIN_RETURN`: 最小年化收益率
> - `SCREEN_MAX_DRAWDOWN`: 最大回撤
> - `SCREEN_MIN_SHARPE`: 最小夏普比率
> - `SCREEN_MIN_SIZE`: 最小基金规模
> - `SCREEN_TOP_N`: 输出前N只基金

### 配置文件

编辑 `src/fund_screener/config/settings.py` 可以调整筛选参数：

```python
SCREENING_CONFIG = {
    # 硬性门槛
    "min_establish_years": 3,      # 最少成立年限
    "min_fund_size": 2.0,          # 最小规模（亿）
    "max_drawdown": 25.0,          # 最大回撤上限
    "min_sharpe": 0.8,             # 最小夏普比率
    "min_annual_return": 5.0,      # 最小年化收益
    "min_manager_exp_years": 1,   # 基金经理最少任职年限
    
    # 评分权重（总和100%）
    "weights": {
        "annual_return_3y": 25,    # 3年年化收益
        "sharpe_ratio": 20,        # 夏普比率
        "calmar_ratio": 15,        # 卡玛比率
        "monthly_win_rate": 10,    # 月度胜率
        "max_drawdown_control": 10,# 回撤控制
        "manager_score": 20,       # 基金经理评分
    },
    
    "top_n": 10,                   # 输出前N只基金
}
```

## 项目结构

```
fund-screener/
├── src/
│   └── fund_screener/
│       ├── __init__.py           # 包初始化
│       ├── __main__.py           # 主入口
│       ├── config/
│       │   ├── __init__.py
│       │   └── settings.py       # 配置文件
│       ├── data/
│       │   ├── __init__.py
│       │   ├── models.py         # 数据库模型
│       │   ├── database.py       # 数据库操作
│       │   └── fetcher.py        # 数据抓取（AKShare）
│       ├── analysis/
│       │   ├── __init__.py
│       │   ├── indicators.py     # 指标计算
│       │   ├── screener.py       # 筛选逻辑
│       │   └── backtest.py       # 回测模块
│       └── report/
│           ├── __init__.py
│           ├── generator.py      # 报告生成
│           └── notifier.py       # 消息推送
├── .env.example                  # 环境变量示例
├── pyproject.toml                # 项目配置（uv规范）
├── requirements.txt              # Python依赖（兼容模式）
└── README.md
```

## 数据源

- **基金数据**：AKShare（免费）
- **基准指数**：沪深300、中证全债等

## 免责声明

- 本工具仅供参考，不构成投资建议
- 基金过往业绩不代表未来表现
- 投资有风险，入市需谨慎

## License

MIT